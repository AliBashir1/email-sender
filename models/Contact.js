...
Contact.prototype.update = function (contactId, ownerId) {
  return new Promise(async (resolve, reject) => {
    try {
      let results = await contactsCollection.findOneAndUpdate({ _id: new ObjectID(contactId), addedBy: new ObjectID(ownerId) }, { $set: { firstName: this.firstName, lastName: this.lastName, email: this.email, lastUpdate: Date.now() } })
...
Contact.deleteContact = function (contactId, ownerId) {
  return new Promise(async (resolve, reject) => {
    try {
      let result = (await contactsCollection.findOneAndDelete({ _id: new ObjectID(contactId), addedBy: new ObjectID(ownerId) })(result.lastErrorObject.n > 0)) ? resolve() : reject()
    } catch {
      reject()
    }
  })
...
Contact.findContactsByOwnerID = function (ownerId) {
  return new Promise(async (resolve, reject) => {
    let contacts
    try {
      let result = await contactsCollection.find({ addedBy: new ObjectID(ownerId) }).toArray()
